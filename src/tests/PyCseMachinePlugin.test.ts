...
async function runAndCollect(code: string, variant = 3) {
  ...
  await evaluate(script, ast, ctx, { isPrelude: true, variant, groups: [] });
  // Reset runtime so collectSnapshots sees a clean run
  const ctx2 = new Context();
  for (const [name, val] of [...math.builtins, ...misc.builtins]) {
    ctx2.nativeStorage.builtins.set(name, val);
  }
  const ast2 = parse(script);
  const control = new Control(ast2);
  const stash = new Stash();
  return collectSnapshots(ctx2, control, stash, 100000, -1, variant, script);
}
...
describe("collectSnapshots", () => {
  it("returns an empty array when maxSnapshots is 0", async () => {
    const ctx = new Context();
    const ast = parse("x = 1\n");
    const snapshots = await collectSnapshots(ctx, new Control(ast), new Stash(), 100000, -1, 3, "x = 1\n", 0);
  ...
  it("respects maxSnapshots cap", async () => {
    ...
    const ast = parse(script);
    const snapshots = await collectSnapshots(ctx, new Control(ast), new Stash(), 100000, -1, 3, script, 2);
